...
if __name__ == '__main__':
    ...
    parser.add_argument("--max_runs", help="Max Runs", type=int, default=10)
    ...
    while i < args.max_runs:
        ...
        with sc2_env.SC2Env(
            ...
            for i in range(args.max_episodes):
                ...
                print('Ep.[{0}] Reward: {1} (Mean: {2}) in {3} seconds, {4} episodes left ({5}), Steps {6}, Loss {7}'
                      .format(i, ep_reward, round(mean_score, 2), round(elapsed, 2), episodes_left,
                              str(datetime.timedelta(seconds=round(seconds_left))), agent.get_steps(), loss))
                ...
                if not args.infinite and mean_score >= args.mean_exit:
                    break

        # Save the Model / States
        df = pd.DataFrame(stats_list, columns=['Episode', 'Reward', 'Seconds', 'Mean', 'Loss'])
        try:
